...
class LogRegManager(models.Manager):
    ...
    def reg_validator(self, postData):
        ...
        alphanumric = re.compile(r'^[0-9a-zA-Z]*$')
        ...
        if len(postData['pw']) < 7:
            errors['pw'] = 'Invalid: Password must be at least 8 characters.'
        if not alphanumric.match(postData['pw']):
            errors['pw'] = 'Invalid: Password can only contain Alphanumeric values.'
        if postData['pw'] != postData['cpw']:
            errors['not_a_match'] = 'Invalid: Passwords do not match!'
